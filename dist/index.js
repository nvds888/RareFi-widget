...
function decodeTxns(b64Txns) {
  return b64Txns.map(b64 => algosdk.decodeUnsignedTransaction(buffer.Buffer.from(b64, 'base64')));
}
...
const MAX_TXN_FEE_UALGOS = 50000;
...
function validateOptInTxn(txnGroup, {
  sender,
  appId
}) {
...
function validateDepositTxn(txnGroup, {
  sender,
  appId,
  appAddress,
  amount,
  ...
  for (let i = 0; i < txnGroup.length; i++) {
    ...
    if (txn.fee !== undefined && Number(txn.fee) > MAX_TXN_FEE_UALGOS) {
      return {
        valid: false,
        error: `Transaction ${i} fee is too high: ${txn.fee} µALGO`
      };
    }
    if (txn.type === 'axfer' && txn.assetTransfer) {
      var _txn$assetTransfer$re;
      const txnAmount = Number(txn.assetTransfer.amount);
      const receiver = (_txn$assetTransfer$re = txn.assetTransfer.receiver) == null ? void 0 : _txn$assetTransfer$re.toString();
      if (txnAmount === 0) {
...
function validateWithdrawTxn(txnGroup, {
  sender,
  appId
}) {
  if (!(txnGroup != null && txnGroup.length)) return {
  ...
  for (let i = 0; i < txnGroup.length; i++) {
    ...
    if (txn.fee !== undefined && Number(txn.fee) > MAX_TXN_FEE_UALGOS) return {
      valid: false,
      error: `Transaction ${i} fee is too high`
    };
...
function validateClaimTxn(txnGroup, {
  sender,
  appId
}) {
  if (!(txnGroup != null && txnGroup.length)) return {
...
function validateCloseOutTxn(txnGroup, {
  sender,
  appId
}) {
  if (!(txnGroup != null && txnGroup.length)) return {
...
function validateSwitchPoolTxn(txnGroup, {
  sender,
  fromAppId,
  toAppId
}) {
...
function DepositPanel({
  ...
  const handleOptIn = async () => {
    ...
    try {
      ...
      const txnGroup = decodeTxns(b64Txns);
      const validation = validateOptInTxn(txnGroup, {
        sender: userAddress,
...
function WithdrawPanel({
  ...
  const handleWithdraw = async e => {
    ...
    try {
      ...
      const b64Txns = res.data.transactions;
      const txnGroup = decodeTxns(b64Txns);
      const validation = validateWithdrawTxn(txnGroup, {
        sender: userAddress,
        appId: selectedPool.appId
      });
      if (!validation.valid) {
...
function ClaimPanel({
  ...
  const handleClaim = async () => {
    ...
    try {
      ...
      const b64Txns = res.data.transactions;
      const txnGroup = decodeTxns(b64Txns);
      const validation = validateClaimTxn(txnGroup, {
        sender: userAddress,
        appId: selectedPool.appId
      });
      if (!validation.valid) {
...
function SwitchPanel({
  ...
  const handleSwitch = async () => {
    ...
    try {
      ...
      const b64Txns = res.data.transactions;
      const txnGroup = decodeTxns(b64Txns);
      const validation = validateSwitchPoolTxn(txnGroup, {
        sender: userAddress,
...
function ExitPanel({
  ...
  const handleExit = async () => {
    ...
    try {
      ...
      const b64Txns = res.data.transactions;
      const txnGroup = decodeTxns(b64Txns);
      const validation = validateCloseOutTxn(txnGroup, {
        sender: userAddress,
        appId: selectedPool.appId
      });
      if (!validation.valid) {
